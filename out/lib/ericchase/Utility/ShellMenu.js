...
import { MenuNavigator, ParseMenu } from "src/lib/ericchase/Utility/Menu.js";

export class ShellMenu {
  $current_item_index = 0;
  $cursor_line_number = 0;
  $navigator = new MenuNavigator({ name: "", label: "", items: [] });
  $parser_cache = ParseMenu();
  $remove_open_handler = undefined;
  $remove_select_handler = undefined;
  constructor(options) {
    this.swap(options);
  }
  get item() {
  ...
  swap(options) {
    ...
    if (options.menu_navigator)
      this.$navigator = options.menu_navigator;
    else if (options.menu)
      this.$navigator = new MenuNavigator(options.menu);
    else
      throw new TypeError(`${ShellMenu.name} requires a Menu or ${MenuNavigator.name} instance.`);
  ...
  $moveCursorToLine(line_number, to_start = false) {
