...
export async function* Async_WebPlatform_DataTransfer_Get_Entries_Generator(dataTransfer) {
  const entries = [];
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    if (entry.isDirectory === true) {
      entries.push(...(await Async_WebPlatform_FileSystemEntry_Read_Directory_Entries(entry)));
    }
  }
}
