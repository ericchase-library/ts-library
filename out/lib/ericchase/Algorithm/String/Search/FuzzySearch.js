...
export class FuzzyMatcher {
  ...
  search(inputText, targetText) {
    ...
    return Array.from(inputWordSet).map(toFuzzyMatchResult).sort((a, b) => a.distance - b.distance);
  }
  searchList(inputTextList, targetText, tolerance = 0) {
    ...
    for (const [inputIndex, inputText] of inputTextList.entries()) {
      if (!inputTextSet.has(inputText)) {
        ...
        matchResultListList.push(this.search(inputText, targetText).map(({ distance }) => ({
          distance,
          inputIndex
        })));
      }
  ...
  searchMultiList(inputTextList, targetText, tolerance = 0) {
    ...
    Array.from(targetWordSet).flatMap((targetWord) => this.searchList(inputTextList, targetWord, tolerance)).forEach(addToMatchResultMap);
