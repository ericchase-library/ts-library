...
export async function NodePlatform_File_Append_Bytes_Async(path: string, bytes: Uint8Array): Promise<void> {
  ...
  return await Core_Error_Fix_Call_Stack_Async(Error().stack, NODE_FS.promises.appendFile(path, bytes));
}
