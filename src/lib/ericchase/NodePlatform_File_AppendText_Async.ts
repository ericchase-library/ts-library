...
export async function NodePlatform_File_AppendText_Async(path: string, text: string): Promise<void> {
  await Core_Error_Fix_Call_Stack_Async(Error().stack, NodePlatform_Directory_Create_Async(NodePlatform_Path_GetDirName(path)));
  return await Core_Error_Fix_Call_Stack_Async(Error().stack, NODE_FS.promises.appendFile(NodePlatform_Path_Join(path), text));
}
