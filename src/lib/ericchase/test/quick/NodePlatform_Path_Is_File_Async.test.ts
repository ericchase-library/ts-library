...
describe(NodePlatform_Path_Is_File_Async.name, () => {
  ...
  test('Should return `true` for symbolic link to file.', async () => {
  ...
  test('Should return `false` for junction to directory.', async () => {
    expect(await NodePlatform_Path_Is_File_Async(temp_junction_to_dir_path)).toBeFalse();
  });
  test('Should return `false` for symbolic link to directory.', async () => {
  ...
  test('Should return `false` for non-existent path.', async () => {
    expect(await NodePlatform_Path_Is_File_Async(temp_nonexistent_path)).toBeFalse();
  });
});
