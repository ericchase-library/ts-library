...
import { NodePlatform_SetupTempDirectory_Async, temp_dir_path, temp_file_path, temp_junction_to_dir_path, temp_nonexistent_path, temp_symboliclink_loop_a_path, temp_symboliclink_loop_b_path, temp_symboliclink_to_dir_path, temp_symboliclink_to_file_path, temp_symboliclink_to_nonexistent_path } from '../test-setup.js';
...
describe(NodePlatform_Path_Get_Stats_SymbolicLink_Async.name, async () => {
  ...
  test('Should NOT throw for directory', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_dir_path)).not.toThrow();
  });
  test('Should NOT throw for junction to directory', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_junction_to_dir_path)).not.toThrow();
  });
  test('Should NOT throw for symbolic link to directory', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_symboliclink_to_dir_path)).not.toThrow();
  });

  test('Should NOT throw for file', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_file_path)).not.toThrow();
  });
  test('Should NOT throw for symbolic link to file', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_symboliclink_to_file_path)).not.toThrow();
  });

  test('Should NOT throw for symbolic link to non-existent path.', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_symboliclink_to_nonexistent_path)).not.toThrow();
  });
  test('Should NOT throw for symbolic link loop.', async () => {
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_symboliclink_loop_a_path)).not.toThrow();
    expect(async () => await NodePlatform_Path_Get_Stats_SymbolicLink_Async(temp_symboliclink_loop_b_path)).not.toThrow();
  });
  ...
  test('Should throw dedicated `TypeError` message if path contains null byte.', async () => {
    try {
      await NodePlatform_Path_Get_Stats_SymbolicLink_Async('\0');
      throw new Error('FAIL-CASE');
    } catch (error: any) {
  ...
  if (process.platform === 'win32') {
    ...
    test('On win32, should return `true` for `isCharacterDevice()` for "NUL".', async () => {
      const stats = await NodePlatform_Path_Get_Stats_SymbolicLink_Async('NUL');
    ...
    test('On win32, should throw `ENAMETOOLONG` if path too long with segments.', async () => {
      ...
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async(path);
        throw new Error('FAIL-CASE');
      } catch (error: any) {
    ...
    test('On win32, should throw `EINVAL` for "CON".', async () => {
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async('CON');
        throw new Error('FAIL-CASE');
      } catch (error: any) {
    ...
    test('On win32, should throw `ENOENT` for "*".', async () => {
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async('*');
        throw new Error('FAIL-CASE');
      } catch (error: any) {
        expect(error.message).toStartWith('ENOENT: no such file or directory');
        expect(error.code).toBe('ENOENT');
    ...
    test('On win32, should throw `ENOENT` if a parent path segment is a file.', async () => {
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async(NODE_PATH.join(temp_file_path, 'file'));
        throw new Error('FAIL-CASE');
      } catch (error: any) {
        expect(error.message).toStartWith('ENOENT: no such file or directory');
        expect(error.code).toBe('ENOENT');
    ...
    test('On posix, should return `true` for `isCharacterDevice()` for "/dev/null".', async () => {
      const stats = await NodePlatform_Path_Get_Stats_SymbolicLink_Async('/dev/null');
      expect(stats.isCharacterDevice()).toBeTrue();
    ...
    test('On posix, should return `true` for `isCharacterDevice()` for "/dev/zero".', async () => {
      const stats = await NodePlatform_Path_Get_Stats_SymbolicLink_Async('/dev/zero');
      expect(stats.isCharacterDevice()).toBeTrue();
    ...
    test('On posix, should return `true` for `isBlockDevice()` for "/dev/sda" or skip.', async () => {
      try {
        const stats = await NodePlatform_Path_Get_Stats_SymbolicLink_Async('/dev/sda');
    ...
    test('On posix, should return `true` for `isBlockDevice()` for "/dev/disk0" or skip.', async () => {
      try {
        const stats = await NodePlatform_Path_Get_Stats_SymbolicLink_Async('/dev/disk0');
        expect(stats.isBlockDevice()).toBeTrue();
    ...
    test('On posix, should throw `ENAMETOOLONG` if path segment too long.', async () => {
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async('a'.repeat(256));
        throw new Error('FAIL-CASE');
      } catch (error: any) {
        expect(error.message).toStartWith('ENAMETOOLONG: name too long');
        expect(error.code).toBe('ENAMETOOLONG');
    ...
    test('On posix, should throw `ENAMETOOLONG` if path too long.', async () => {
      ...
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async(path.repeat(256));
        throw new Error('FAIL-CASE');
      } catch (error: any) {
        expect(error.message).toStartWith('ENAMETOOLONG: name too long');
        expect(error.code).toBe('ENAMETOOLONG');
    ...
    test('On posix, should NOT throw `ENAMETOOLONG` if path segment length is 1 character less than too long.', async () => {
      try {
        await NodePlatform_Path_Get_Stats_SymbolicLink_Async('a'.repeat(256 - 1));
        throw new Error('FAIL-CASE');
      } catch (error: any) {
        expect(error.message).not.toStartWith('ENAMETOOLONG: name too long');
        expect(error.code).not.toBe('ENAMETOOLONG');
